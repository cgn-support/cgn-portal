<div>
    ...
    @if (empty($availableYears))
        ...
        <div class="text-center py-16">
            ...
            <p class="mt-2 text-gray-500">Your marketing reports will appear here once they're published by your account
                manager.</p>
        </div>
    @else
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @foreach ($monthlyReports as $monthData)
                <div
                    class="agency-card p-6 text-center relative {{ $monthData['report'] ? 'hover:shadow-lg cursor-pointer' : 'opacity-60' }} transition-all duration-200">
                    @if ($monthData['report'])
                        <!-- Published Report -->
                        <a href="{{ route('project.report', ['uuid' => $project->id, 'report_id' => $monthData['report']->id]) }}"
                            ...
                            <div class="mb-4">
                                <div
                                    class="w-16 h-16 bg-gradient-to-br from-orange-400 to-orange-600 rounded-full mx-auto flex items-center justify-center">
                                    <span class="text-white font-bold text-lg">{{ $monthData['report_number'] }}</span>
                                </div>
                            </div>
                            <h3 class="text-lg font-semibold text-gray-900 mb-2">Report
                                {{ $monthData['report_number'] }}</h3>
                    ...
                    @else
                        ...
                        <h3 class="text-lg font-semibold text-gray-500 mb-2">Report {{ $monthData['report_number'] }}
                        </h3>
                        ...
                        @else
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 mb-3">
                                <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd"
                        ...
                        <div
                            class="bg-gray-100 text-gray-500 px-4 py-2 rounded-full text-sm font-medium cursor-not-allowed">
                            Not Available
                        </div>
                    @endif
                </div>
