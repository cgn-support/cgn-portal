<div>
    <div class="flex items-center justify-between mb-6">
        ...
        @if ($trackingAvailable)
            <button wire:click="refreshMetrics" wire:loading.attr="disabled"
                class="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-full text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 disabled:opacity-50">
                <svg wire:loading.remove wire:target="refreshMetrics" class="w-4 h-4 mr-2" fill="none"
                    stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                </svg>
                <svg wire:loading wire:target="refreshMetrics" class="animate-spin w-4 h-4 mr-2" fill="none"
                    viewBox="0 0 24 24">
    ...
    @if (!$trackingAvailable)
        <div class="bg-yellow-50 border border-yellow-200 rounded-full p-4 mb-6">
            <div class="flex">
                ...
                <div>
                    <p class="text-sm text-yellow-800">
                        <strong>Limited Tracking Available:</strong> Some automated metrics may not be available for
                        this project. Manual metrics can still be entered via the admin panel.
                    </p>
                </div>
            </div>
        </div>
                ...
                <div class="flex items-center justify-center mb-1">
                    @if (($comparisons['form_submissions']['direction'] ?? 'unchanged') === 'increase')
                        ...
                        <p class="text-lg font-bold text-green-600">
                            {{ $comparisons['form_submissions']['display'] ?? '0%' }}</p>
            ...
            <div class="text-center">
                <div class="flex items-center justify-center mb-1">
                    @if (($comparisons['total_citations']['direction'] ?? 'unchanged') === 'increase')
                        ...
                        <p class="text-lg font-bold text-green-600">
                            {{ $comparisons['total_citations']['display'] ?? '0%' }}</p>
            ...
            <div class="text-center">
                <div class="flex items-center justify-center mb-1">
                    @if (($comparisons['total_reviews']['direction'] ?? 'unchanged') === 'increase')
                        ...
                        <p class="text-lg font-bold text-green-600">
                            {{ $comparisons['total_reviews']['display'] ?? '0%' }}</p>
